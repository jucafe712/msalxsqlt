...
class login:
    def __init__(self, root):
        ...
        #self.icon_side=PhotoImage(file="images/img12.jpg")

        #self.phone_image=ImageTk.PhotoImage(file="images/cat2.png")
        #self.lbl_Phone_image=Label(self.root,image=self.phone_image,bd=0)
        
        self.bg=ImageTk.PhotoImage(file="images/cat2.jpg")
        ...
        ).place(x=310, y=410, width=180, height=40)
    ...
    def login(self):
        ...
        #con.load_extension('/home/jcf/.local/lib64/python3.9/libsqlcipher.so')
        con.execute("PRAGMA key='123'")
        cur = con.cursor()
        try:
            if self.employee_id.get() == "" or self.password.get() == "":
                messagebox.showerror(
                    "Error!", "Llene todo los campos!!!", parent=self.root
                )
            else:
                cur.execute(
                ...
                if user == None:
                    messagebox.showerror(
                        "Error!", "Nombre / Clave:Usuario Invalido!!!", parent=self.root
                    )
                else:
                    # print(user)
    ...
    def forget_window(self):
        ...
        try:
            if self.employee_id.get() == "":
                messagebox.showerror(
                    "Error!", "Id funcionario Requerido!!!", parent=self.root
                )
            else:
                cur.execute(
                    "select email from employee where eid=?", (self.employee_id.get(),)
                )
                email = cur.fetchone()
                if email == None:
                    messagebox.showerror(
                        "Error!", "ID funcionario incorrecto!!!", parent=self.root
                    )
                else:
                    # ====Forget Window=====
                ...
                if chk == "f":
                    messagebox.showerror(
                        "Atencion!!", "Error de conexion!!!", parent=self.root
                    )
                else:
                    ...
                    lbl_reset = Label(
                        self.forget_win,
                        text="Ingrese OTP Enviado a correo",
                        font=("times new roman", 15),
                    ).place(x=20, y=60)
                    txt_reset = Entry(
                        self.forget_win,
                        ...
                        font=("times new roman", 15),
                        bg="lightyellow",
                    ).place(x=20, y=100, width=250, height=30)

                    lbl_new_pass = Label(
                        self.forget_win,
                        text="Ingrese Nueva Clave",
                        font=("times new roman", 15),
                    ).place(x=20, y=160)
                    txt_new_pass = Entry(
                        self.forget_win,
                        textvariable=self.var_new_pass,show="?",
                        font=("times new roman", 15),
                        bg="lightyellow",
                    ).place(x=20, y=190, width=250, height=30)

                    lbl_c_pass = Label(
                        self.forget_win,
                        text="Reescriba Clave",
                        font=("times new roman", 15),
                    ).place(x=20, y=225)
                    txt_c_pass = Entry(
                        self.forget_win,
                        textvariable=self.var_conf_pass,show="?",
                        font=("times new roman", 15),
                        bg="lightyellow",
                    ).place(x=20, y=255, width=250, height=30)
    ...
    def update_password(self):
        if self.var_new_pass.get() == "" or self.var_conf_pass.get() == "":
            ...
            messagebox.showerror(
                "Atencion!!!", "Clave NO es igual!!!", parent=self.forget_win
            )
        else:
            con = sqlite3.connect(database=r"msa.db")
            cur = con.cursor()
            try:
                cur.execute(
                    "Update employee SET pass=? where eid=?",
    ...
    def validate_otp(self):
        if int(self.otp) == int(self.var_otp.get()):
            ...
            messagebox.showerror(
                "Atencion!!", "Codigo OTP Invalido!!", parent=self.forget_win
            )
    ...
    def send_email(self, to_):
        ...
        subj = "MSA-Reestablecer Clave OTP"
        msg = f"Apreciado funcionario MSA,\n\nSu codigo OTP, para reestablecer clave es: {str(self.otp)}.\n\n Cordial Saludo,\nEquipo Soporte MSA"
        msg = "Subject:{}\n\n{}".format(subj, msg)
